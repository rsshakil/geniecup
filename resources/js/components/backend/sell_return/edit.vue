...
<div class="main-content-container container-fluid" style="padding-left:30px;">
    ...
    <!-- Default Light Table -->
        ...
        <div class="col-md-12">
          <router-link :to="'/sell/'+$route.params.id" class="btn btn-primary" style="width:49%;float:left;margin-right:1%;">Back</router-link>
          <button class="btn btn-primary" @click="updateInvoice" style="width:50%;float:left;">Update</button>
        </div>
...
export default {
  ...
  methods: {
    updateInvoice(){
      ...
      console.log(this.product_list_arr_orginal);
       var post_data = {
        client_id: Globals.user_info_client_id,
        sell_info:this.product_list_arr,
        orginal_sell_info:this.product_list_arr_orginal,
      };
      axios.post(this.BASE_URL+"api/sell_invoice_update",post_data)
        .then(({ data }) => {
          this.getDetails(this.$route.params.id);
          Swal.fire({
                icon: 'success',
                title: 'Invoice update',
                text: "Invoice update success"
            });
        })
        .catch(() => {
          console.log("Error...");
    ...
    getDetails(id){
      ...
      axios.post(this.BASE_URL+"api/sell_history",post_data)
        .then(({ data }) => {
            ...
            this.product_list_arr_orginal = data.products;
