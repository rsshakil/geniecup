<template>
  <div class="p-3 bg-white shadow rounded-lg">
    ...
    <button v-if="this.imageSrc" v-on:click.prevent="cropImage">Crop</button>
    <button v-if="this.croppedImageSrc" v-on:click.prevent="uploadImage">Upload</button>
  </div>
</template>
    ...
    methods: {
      ...
      uploadImage() {
        ...
        this.$refs.cropper.getCroppedCanvas().toBlob(function (blob) {
          ...
          axios
            .post(Globals.base_url+"/api/store", formData)
            .then(response => {
